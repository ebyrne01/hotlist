...
function toHarvestBook(row: Record<string, string>) {
  ...
  return {
    ...
    goodreadsId: row.goodreadsId || row.goodreads_id || row.GoodreadsID || null,
    asin: row.asin || row.ASIN || null,
    seriesName: row.seriesName || row.series_name || row.Series || null,
    seriesPosition: parseInt_(row.seriesPosition || row.series_position || row.SeriesPosition),
    coverUrl: row.coverUrl || row.cover_url || row.CoverURL || null,
    goodreadsRating: parseNum(row.goodreadsRating || row.goodreads_rating || row.GoodreadsRating),
    goodreadsRatingCount: parseInt_(row.goodreadsRatingCount || row.goodreads_rating_count || row.GoodreadsRatingCount),
