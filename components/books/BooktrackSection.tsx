...
const STORAGE_KEY = "hotlist-booktrack-expanded";
...
export default function BooktrackSection({
  ...
  // Restore expand/collapse preference from localStorage
  useEffect(() => {
    try {
  ...
  // Don't render if we have neither playlists nor a vibes prompt
  ...
  return (
    <div>
      ...
      >
        <span className="text-xs font-mono text-muted uppercase tracking-wide flex items-center gap-1.5">
          <SpotifyLogo size={14} />
          Booktrack
        </span>
        <ChevronDown
      ...
      {!expanded && (
        <button
          onClick={toggleExpanded}
          className="mt-1 text-xs font-mono text-stone-400 text-left"
        >
          {hasPlaylists ? "Listen to a playlist for this book" : "Get a custom playlist for this book"}
        </button>
      )}
